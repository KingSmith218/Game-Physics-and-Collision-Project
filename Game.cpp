	...
	while (!collisionData.empty())
	{
		...
		if (data->contact != nullptr)
			resolver->resolveContacts(data->contact, data->maxNumContacts, dt);
				params = new Parameters(10, vec2(data->contact->getManifold()), vec2(0));
		//Ground   // work out how to calculate contact point, how to do warm starting, object shattering, also where to calculate angular velocity and implement it
